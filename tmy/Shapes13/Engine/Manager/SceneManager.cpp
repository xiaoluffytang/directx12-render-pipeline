...
void SceneManager::InitEnd()
{
	manager->materialManager->GetMaterial("screen", true)->DiffuseSrvHeapIndex =     //屏幕后处理
		manager->textureManager->screenPostMap->texture->srvIndex;
	manager->materialManager->GetMaterial("skyBox", true)->DiffuseSrvHeapIndex =    //天空盒
		manager->textureManager->getTexture("../../Textures/grasscube1024.dds")->srvIndex; 
	manager->materialManager->GetMaterial("opaque_back", true)->DiffuseSrvHeapIndex =    //普通的不透明物体渲染材质
		manager->textureManager->getTexture("../../Textures/grass.dds")->srvIndex;
	manager->materialManager->GetMaterial("specular", true)->DiffuseSrvHeapIndex =     //反射镜面材质
		manager->textureManager->getTexture("../../Textures/ice.dds")->srvIndex;
	manager->materialManager->GetMaterial("uiSprite", true)->DiffuseSrvHeapIndex =    //普通ui图片渲染--显示树图片
		manager->textureManager->getTexture("../../Textures/tree01S.dds")->srvIndex;
	manager->materialManager->GetMaterial("uiShadowMap", true)->DiffuseSrvHeapIndex =    //普通ui图片渲染--显示阴影贴图
		manager->textureManager->shadowMap->shadowMap->srvIndex;
	manager->materialManager->GetMaterial("uiScreenPost", true)->DiffuseSrvHeapIndex =    //普通ui图片渲染--显示用于屏幕后处理的贴图
		manager->textureManager->screenPostMap->texture->srvIndex;
	manager->materialManager->GetMaterial("uiDepth", true)->DiffuseSrvHeapIndex =    //普通ui图片渲染--显示深度图
		manager->textureManager->depthStencil->srvIndex;
	manager->materialManager->GetMaterial("font", true)->DiffuseSrvHeapIndex =     //普通ui图片渲染--显示字体贴图
		manager->fontManager->font->texture->srvIndex;
	manager->materialManager->GetMaterial("cubeTargetReflect", true)->DiffuseSrvHeapIndex =    //环境映射
		manager->textureManager->cubeTarget->cubeMap->srvIndex;
	manager->materialManager->GetMaterial("water", true)->DiffuseSrvHeapIndex =      //普通河流渲染
		manager->textureManager->getTexture("../../Textures/NoiseTex.dds")->srvIndex;
	manager->materialManager->GetMaterial("font3d", true)->DiffuseSrvHeapIndex =     //渲染3d字体
		manager->textureManager->getTexture("../../Textures/stone.dds")->srvIndex;
	manager->materialManager->GetMaterial("skull", true)->DiffuseSrvHeapIndex =     //渲染骨头的粒子特效
		manager->textureManager->particle->texture->srvIndex;
	manager->materialManager->GetMaterial("particlePosOffsetTex", true)->DiffuseSrvHeapIndex =   //普通ui图片渲染--显示骨头的粒子特效位置贴图
		manager->textureManager->particle->texture->srvIndex;
	manager->materialManager->GetMaterial("moutain", true)->DiffuseSrvHeapIndex =    //渲染程序化地形-山
		manager->textureManager->getTexture("../../Textures/stone.dds")->srvIndex;
	manager->materialManager->GetMaterial("uiText", true)->DiffuseSrvHeapIndex =    //渲染文字
		manager->fontManager->font->texture->srvIndex;
}
...
{
	manager->textureManager->LoadTexture3D("../../Textures/grasscube1024.dds");
	std::vector<string> paths = { "grass" ,"ice","water1" ,"tree01S" ,"NoiseTex" ,"stone", "star"};
	for (int i = 0; i < paths.size(); i++)
	{
		manager->textureManager->LoadTexture2D("../../Textures/"+ paths[i] + ".dds");
	}
}
...
{
	int setAside = manager->textureManager->setAside + 2;   //前多少张纹理是特殊的程序纹理 
	std::vector<string> materialNames = { "screen","skyBox" ,"opaque_back","specular" ,"uiSprite" ,"uiShadowMap" ,"uiScreenPost","uiDepth","font",
		"cubeTargetReflect" ,"water","font3d","skull","particlePosOffsetTex","moutain","uiText" };
	std::vector<string> psoNames = { "screenDefault","skyBox" ,"opaque_back","Transparent" ,"uiSprite" ,"uiSprite" ,"uiSprite","uiSprite","uiSprite",
		"cubeTargetReflect" ,"lake","opaque_None" ,"skull","uiSprite","opaque_back","uiText"};
...
{
	std::ifstream fin(manager->getPathPrefix(filepath));
	if (!fin)
	{
	...
	manager->textureManager->particle->InitPos();
	fin >> ignore;
	fin >> ignore;
	fin >> ignore;
...
void SceneManager::BuildM3dModel(const string& mSkinnedModelFilename)
{
	...
	M3DLoader m3dLoader;
	m3dLoader.LoadM3d(manager->getPathPrefix(mSkinnedModelFilename), vertices, indices,
	...
	for (UINT i = 0; i < mSkinnedMats.size(); ++i)
	{
		...
		material->DiffuseSrvHeapIndex = manager->textureManager->LoadTexture2D(path1);
		material->NormalSrvHeapIndex = manager->textureManager->LoadTexture2D(path2);
	}
...
{
	string path = manager->getPathPrefix("../../font/simhei.ttf");
	Font3D* font3d = new Font3D(path);
...
{
	ModelObj* model = new ModelObj(manager->getPathPrefix(path), fToTri);
	model->readfile();
	...
	for (auto& newmtl : model->newmtls)
	{
		...
		if (texturePath != "")
		{
			int index = manager->textureManager->LoadTexture2D(textureOppPath + texturePath);
...
void SceneManager::OnKeyboardInput(const GameTimer& gt)
{
	...
	if (GetAsyncKeyState('D') & 0x8000)
		camera->Strafe(speed*dt);
	manager->cameraManager->mainCamera->UpdateViewMatrix();
}