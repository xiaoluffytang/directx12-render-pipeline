...
void Font::DrawSimpleText(const std::string& str)
{
	...
	UpdateSubresources(commandList, texture->Resource.Get(), fontTempMap, 0, 0, 1, &fontTextureData);
	commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(texture->Resource.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
}
