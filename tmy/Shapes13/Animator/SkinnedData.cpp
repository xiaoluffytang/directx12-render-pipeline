...
void SkinnedData::BuildAniTex(ID3D12Device* md3dDevice, ID3D12GraphicsCommandList* commandList
	, ID3D12DescriptorHeap* heap, int startIndex, int mCbvSrvUavDescriptorSize)
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE frameTexCpuSrvHandle(heap->GetCPUDescriptorHandleForHeapStart(), startIndex, mCbvSrvUavDescriptorSize);
	this->frameTexGpuSrvHandle = CD3DX12_GPU_DESCRIPTOR_HANDLE(heap->GetGPUDescriptorHandleForHeapStart(), startIndex, mCbvSrvUavDescriptorSize);
	...
	UpdateSubresources(commandList, frameTex, temp, 0, 0, 1, &fontTextureData);
	commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(frameTex, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
	...
	md3dDevice->CreateShaderResourceView(frameTex, &srvDesc, frameTexCpuSrvHandle);

	skin.aniTexIndex = startIndex;
